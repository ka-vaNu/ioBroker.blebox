const datapoints = {
    ...
    "saunabox#heat.minimumTemp": {
        path: "heat.minimumTemp", factor: 0.01, type: "state", common: { name: "saunaBox's minimal Temperature for regulator, minimum value: -55", type: "number", role: "value.temperature", read: true, write: false, unit: "°C" }
    },
    "saunabox#heat.maximumTemp": {
        path: "heat.maximumTemp", factor: 0.01, type: "state", common: { name: "saunaBox's maximal Temperature for regulator, maximum value: 125", type: "number", role: "value.temperature", read: true, write: false, unit: "°C" }
    },
    "saunabox#heat.state": {
        path: "heat.state", type: "state", common: { name: "state of regulator (0 - off, 1 - on)", type: "number", role: "value", read: true, write: false }
    },
    "saunabox#heat.desiredTemp": {
        path: "heat.desiredTemp", factor: 0.01, type: "state", common: { name: "regulator's desired temperature, value between max&min from settings", type: "number", role: "value.temperature", read: true, write: false, unit: "°C" }
    },
    ...
    "saunabox#heat.sensors[0].value": {
        path: "heat.sensors.0.value", factor: 0.01, type: "state", common: { name: "Value of measured data, where null = Error and Temperature in Celsius degrees.", type: "number", role: "value.temperature", read: true, write: false, unit: "°C" }
    },
    "saunabox#heat.sensors[0].trend": {
        path: "heat.sensors.0.trend", type: "state", common: { name: "Trend of measured data, where 0 = no data, 1 = sidewave, 2 = Downward, 3 = Upward", type: "number", role: "value", read: true, write: false }
    },
    "saunabox#heat.sensors[0].state": {
        path: "heat.sensors.0.state", type: "state", common: { name: "Status of sensor, where 1 = Measure in progress, 2 = Active Mode, 3 = Error", type: "number", role: "value", read: true, write: false }
    },
    "saunabox#heat.sensors[0].elapsedTimeS": {
        path: "heat.sensors.0.elapsedTimeS", type: "state", common: { name: "Time in seconds that has elapsed since last valid measurement occurred, where -1 = Error, 0 = Fresh data", type: "number", role: "value", read: true, write: false }
    }
};
