const datapoints = {
    ...
    "tempsensor#tempSensor.sensors[0].elapsedTimeS": {
        path: "tempSensor.sensors.0.elapsedTimeS", type: "state", common: { name: "Time in seconds that has elapsed since last valid measurement occurred, where -1 = Error, 0 = Fresh data", type: "number", role: "value", read: true, write: false }
    },
    "tempsensor#tempSensor.sensors[0].state": {
        path: "tempSensor.sensors.0.state", type: "state", common: { name: "Status of sensor, where 1 = Measure in progress, 2 = Active Mode, 3 = Error", type: "number", role: "value", read: true, write: false }
    },
    "tempsensor#tempSensor.sensors[0].trend": {
        path: "tempSensor.sensors.0.trend", type: "state", common: { name: "Trend of measured data, where 0 = no data, 1 = sidewave, 2 = Downward, 3 = Upward", type: "number", role: "value", read: true, write: false }
    },
