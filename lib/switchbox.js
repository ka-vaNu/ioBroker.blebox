const datapoints = {
    ...
    "switchbox#settings.relays[0].stateAfterRestart": {
        path: "settings.relays.0.stateAfterRestart", type: "state", common: { name: "State of relay after reboot/reset. Where: 0 - OFF, 1 - ON, 2 - Last state, 3 - Opposite state.", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#settings.relays[0].defaultForTime": {
        path: "settings.relays.0.defaultForTime", type: "state", common: { name: "Time in seconds for button \"turn on for time\" in wBox App, where 0 == disabled button", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#settings.relays[0].powerMeasuring.enabled": {
        path: "settings.relays.0.powerMeasuring.enabled", type: "state", common: { name: "Allows to enable or disable power measuring module (powerConsumption and activePower sensor). If hardware doesn't support power measuring then this object doesn't exist. 0 - disabled, 1 - enabled", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#relays[0].relay": {
        path: "relays.0.relay", type: "state", common: { name: "ID - relay number", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#relays[0].state": {
        path: "relays.0.state", type: "state", common: { name: "State of relay, 0 - OFF, 1 - ON", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#relays[0].stateAfterRestart": {
        path: "relays.0.stateAfterRestart", type: "state", common: { name: "State of relay after reebot/reset, 0 - OFF, 1 - ON, 2 - Last State, 3 - Opposed state", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#relays[0].defaultForTime": {
        path: "relays.0.defaultForTime", type: "state", common: { name: "Time in seconds for button \"turn on for time\" in wBox App, where 0 == disabled button", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#powerMeasuring.enabled": {
        path: "powerMeasuring.enabled", type: "state", common: { name: "Indicates whether the function is disabled or enabled, 0 - disabled, 1 - enabled", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#powerMeasuring.powerConsumption[0].periodS": {
        path: "powerMeasuring.powerConsumption.0.periodS", type: "state", common: { name: "Period of measurement in seconds - applies to the \"value\" field", type: "number", role: "value", read: true, write: false }
    },
    "switchbox#powerMeasuring.powerConsumption[0].value": {
        path: "powerMeasuring.powerConsumption.0.value", type: "state", common: { name: "Energy consuption in kWh. Always 3 decimal places", type: "number", role: "value", read: true, write: false }
    },
    ...
    "switchbox#powerMeasuring.sensors[0].value": {
        path: "powerMeasuring.sensors.0.value", type: "state", common: { name: "Current value of measurement - power of connected device (active power counted in Watt)", type: "number", role: "value", read: true, write: false }
    }
};
