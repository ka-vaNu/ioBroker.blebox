const datapoints = {
    ...
    "settings.relays[0].stateAfterRestart": {
        type: "state", common: { name: "State of relay after reebot/reset. Where: 0 - OFF, 1 - ON, 2 - Last state, 3 - Opposite state.", type: "number", role: "text", read: true, write: false }
    },
    ...
    "settings.relays[0].powerMeasuring.enabled": {
        type: "state", common: { name: "Allows to enable or disable power measuring module (powerConsumption and activePower sensor). If hardware doesn't support power measuring then this object doesn't exist. 0 - disabled, 1 - enabled", type: "number", role: "text", read: true, write: false }
    },
    "relays[0].relay": {
