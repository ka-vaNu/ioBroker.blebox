...
async function setIobStates (device, values) {
    ...
    for (const key in values) {
        ...
        if (iob.extLog) iob.log.info("setIobStates datapoints: " + JSON.stringify(iob.datapoints[device.dev_name]));
        if (Object.prototype.hasOwnProperty.call(iob.datapoints[device.dev_name], search)) {
            ...
            if (iob.extLog) iob.log.info("setIobStates: " + JSON.stringify(device.dev_name + "." + path) + " = " + JSON.stringify(value));
            if (path) {
