...
async function setIobStates(device, values) {
    ...
    for (const key in values) {
        iob.log.info("setIobStates key: " + key);
        iob.log.info("setIobStates datapoints: " + JSON.stringify(iob.datapoints[device.name]));
        if (Object.prototype.hasOwnProperty.call(iob.datapoints[device.name], key)) {
            ...
            if (path) {
                if (Object.prototype.hasOwnProperty.call(deviceDatapoints[key], "factor")) {
                    iob.setState(device.name + "." + path, value * deviceDatapoints[key].factor);
                } else {
